...
function setup() {
  ...
  trex = createSprite(displayWidth/7,displayHeight/2,20,50);
  ...
  invisibleGround = createSprite(displayWidth/2,displayHeight/2,displayWidth,10);
...
function draw() {
  //trex.debug = true;
  background(255);
  text("Score: "+ score, displayWidth-200,displayHeight/6);
  
  if (gameState===PLAY){
    score = score + Math.round(getFrameRate()/60);
    ground.velocityX = -(6 + 3*score/100);
    
    if(keyDown("space") && trex.y >= displayHeight/2-100) {
      trex.velocityY = -12;
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random(displayHeight/2-200,displayHeight/2-150));
